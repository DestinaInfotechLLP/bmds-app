...
const UpdateCar = ({ navigation, route }) => {
	...
	useEffect(() => {
		...
		setTimeout(async () => {
			await axios
				.get(`${APP_URL}/admin/sell/show/${id}`, {
					headers: {
				...
				.then((response) => {
					...
					setBrandSelect(data.car_brand_id);
					setModelSelect(data.car_model_id);
					setVariantSelect(data.car_id);
					setFuelType(data.fuel_type);
					setTransmission(data.transmission);
					setYear(data.year);
					setKm(data.km_driven);
					setOwner(data.number_of_owners);
					setColor(data.color);
					setTitle(data.title);
	...
	const handlerSubmitForm = async () => {
		...
		axios
			.post(`${APP_URL}/admin/sell/update/${id}`, data, {
				headers: {
					Authorization: `Bearer ${token}`,
					Accept: "application/json",
				...
				<ScrollView>
					<View style={styles.formContainer}>
						<View style={styles.textHeaderContainer}>
							<Text style={styles.textHeader}>Update Your Car Details</Text>
						</View>
						<Picker
